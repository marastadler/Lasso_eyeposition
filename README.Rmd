...
```
model for the first 30 frames (the runtime is quite long)
```{r cache = TRUE}
Lasso_Group_vid18 <- 
...
plot_ly(x = ~1:nrow(beta_norm), y = ~beta_norm[, 1], name = 'Static', 
  ...
  add_trace(y = ~beta_norm[, 6], name = 'AOI2 Treat', color = I("brown"), 
            mode = 'lines') %>%
  add_trace(y = ~beta_norm[, 7], name = 'Static Control', 
            line = list(widthh=0.5, dash="dot"),
            color = I("blue"), mode = 'lines') %>%
  add_trace(y = ~beta_norm[, 8], name = 'Dynamic Control', 
            line = list(widthh=0.5, dash="dot"),
            color = I("orange"), mode = 'lines') %>%
  add_trace(y = ~beta_norm[, 9], name = 'Center Bias Control',
            line = list(widthh=0.5, dash="dot"),
            color = I("darkgreen"), mode = 'lines') %>%
  add_trace(y = ~beta_norm[, 10], name = 'Uniform Control', 
            line = list(widthh=0.5, dash="dot"),
            color = I("red"), mode = 'lines') %>%
  add_trace(y = ~beta_norm[, 11], name = 'AOI1 Control', 
            line = list(widthh=0.5, dash="dot"),
            color = I("purple"), mode = 'lines') %>%
  add_trace(y = ~beta_norm[, 12], name = 'AOI2 Control', 
            line = list(widthh=0.5, dash="dot"),
